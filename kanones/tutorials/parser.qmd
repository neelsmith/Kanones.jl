...
The result is a Vector of analyses. Each `Analysis` includes an  *morphological form* object and an identifier for a *lexeme* (or vocabulary item). You can use the `greekForm` and `lexemeurn` functions to extract these from an `Analysis`; for a human-readable string value, use the `label` function on the result.  E.g., label the first form in the result:
...
To parse a list of words, use the `parsewordlist` function.

```{julia}
p2 = Kanones.coreparser(; atticonly = true)
